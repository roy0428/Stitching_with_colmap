...
def get_color_for_3Dpoint_in_plane(plane_point, cams, images,image_w, image_h, intrinsics):
    ...
    for key in images:
        ...
        K, dist = COLMAP_functions.build_intrinsic_matrix(intrinsics[1]) #[key]
...
def get_color_for_3Dpoint_in_plane_new(plane_point, cams, images,image_w, image_h, intrinsics):
    # t = time.time()
    colors = []
    X = np.asarray([plane_point[0], plane_point[1], plane_point[2], 1])
    #get camera intrinsics
    ...
    if pix_x >= w or pix_x < 0 or pix_y >= h or pix_y < 0:
        ...
        colors.append(images[pix_y,pix_x,:3])
...
def get_color_for_virtual_pixel(images,Pvirtual,pixelpoint,plane, cams,intrinsics,w_virtual,h_virtual,K_virt):
    ...
    color = get_color_for_3Dpoint_in_plane_new(plane_point, cams, images,w_virtual,h_virtual, intrinsics)
    return color
            ...
            for x in range(0, w_virtual):
                for index in images:
                    ...
                    if pix_x >= w_real[key] or pix_x < 0 or pix_y >= h_real[key] or pix_y < 0:
                        ...
                        stitched_image[y, x, :3] = images[index][pix_y, pix_x,:3] #color_images[index][y, x, :3]
                        break
        return color_images, stitched_image
...
def get_top_view(plane,Pvirtual,w_virtual,h_virtual,images,cams,intrinsics,K_virt,H):
    top_view_images = {}
    # for key in images:
    # color_images[key] = np.zeros((h_virtual, w_virtual,  3))
    ...
    for i in range(len(images)):  
        ...
        for y in range(0,h_virtual):
            ...
            for x in range(0, w_virtual):     
                color = get_color_for_virtual_pixel(images[i+1], Pvirtual[i], [x, y], plane,cams[i+1],intrinsics,w_virtual,h_virtual,K_virt)    
                #for j, key in enumerate(images[i+1]):     
                    # color_images[key][y, x, :] = color[i]
                if color[0] is not None:   
                    top_view_image[y,x,:] = color[0]
        top_view_images[i] = top_view_image
        output = 'result-%d' % i + '.jpg'
        cv.imwrite(output, top_view_images[i][...,::-1])
        
    return top_view_images
    
def get_top_view_every_ten(plane,Pvirtual,w_virtual,h_virtual,images,cams,intrinsics,K_virt,H):
    top_view_images = {}
    # for key in images:
    # color_images[key] = np.zeros((h_virtual, w_virtual,  3))
    #top_view_image = np.zeros((h_virtual,w_virtual, 3))
...
def create_virtual_camera(camera_matrices,plane):
    ...
    return Pvirt
def create_virtual_camera_for_each_images(camera_matrices,plane):
    centers = {}
    #axes = {}
    Pvirt = {}
    for index,cam in enumerate(camera_matrices):
    ...
    for i in range(len(centers)):
        virt_center = centers[i]
        tvirt = np.asarray(np.matmul(-Rvirt,np.asarray([virt_center[0][0],virt_center[1][0],virt_center[2][0]])),dtype='float')
        Pvirt[i] = np.column_stack((Rvirt,tvirt))

    return Pvirt

def create_virtual_camera_for_each_images_based_on_center(camera_matrices,plane):
    centers = {}
    all_center = 0
    #axes = {}
    Pvirt = {}
    for index,cam in enumerate(camera_matrices):
        cam_center, principal_axis = estimate_plane.get_camera_center_and_axis(camera_matrices[cam]['P'])
        centers[index] = cam_center
        all_center += cam_center
        #if index == 0:
            #virt_center = cam_center
        #else:
            #virt_center += cam_center
        #axes[index] = principal_axis
    virt_center = all_center/len(camera_matrices)
